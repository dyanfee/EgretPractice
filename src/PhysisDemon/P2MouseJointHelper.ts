...
class P2MouseJointHelper {
    ...
    public constructor(stageRef: egret.Stage,pSceneCtn: egret.DisplayObjectContainer,pWorld: p2.World,hasDisplaySkin: Boolean = true) {
        ...
        if(hasDisplaySkin) {//是否有显示皮肤
            this.mouseBody = PhysicsDemo.P2Tool.addOneBox(this.world,this.sceneCtn,400,400,20,20,0,p2.Body.KINEMATIC);//mouseBody
    ...
    private onStageTouchBegin(e: egret.TouchEvent): void {
        ...
        var p2x: number = PhysicsDemo.P2Tool.convertEgretValueToP2(e.stageX);
        var p2y: number = PhysicsDemo.P2Tool.convertEgretY_To_P2Y(e.stageY);
