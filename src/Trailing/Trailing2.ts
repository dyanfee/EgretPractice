class Trailing2 extends MyComponent {
	public constructor() {
		super();
	}
	myload() {
	...
	private _oldTriangle: egret.Shape;
	private createTriangle(x, y, height, angle2) {
		let triangle: egret.Shape = new egret.Shape();
	...
	private createMultiRect(dis, angle, angle2, y) {
		...
		for (let i = 0; i < len; i++) {
			let disTemp = lineW * (i) + lineW / 2;			// console.log(disTemp);
			let xTemp = disTemp * Math.cos(angle);
			let yTemp = disTemp * Math.sin(angle);
			yTemp *= y < 0 ? -1 : 1;
			let x1 = this._oldTouchX + xTemp;
